...
function getOnlyFutureEvents(events) {
  ...
  events.forEach(event => {
    if (event.startDate) {
      const isFutureDay = isAfter(event.startDate, today);
      if (isFutureDay) {
        response.push(event);
      }
    } else {
...
function getOngoingEvents(events) {
  ...
  const today = new Date();
  events.forEach(event => {
    const hasStarted = isSameDayOrBefore(event.startDate, today);
    if (hasStarted && event.endDate) {
...
function isSameDayOrBefore(dateA, dateB) {
  if (!dateA || !dateB) {
    return false;
  }
