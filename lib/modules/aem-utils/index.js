...
function construct(self, options) {
  ...
  function getHeroImageUrlFromMetadata(options) {
    const defaultResponse = `${ options.baseUrl }/img/opengraph.jpg`;

    if (options.isUsingRemoteImage) {
      return options.remoteImageLocation;
    }
    ...
    if (options.heroImage) {
      ...
      if (aposImageData) {
        ...
        if (imageData.url) {
          return `${ options.baseUrl }${ imageData.url }`;
        } else {
