...
module.exports = {
  ...
  construct: (self, options) => {
    ...
    self.on('apostrophe-pages:beforeSend', 'fetchAEMPeople', async function(req) {
      ...
      // sorted by title
      const publicPeopleByTitle = await apos.docs.getManager(type)
        .find(req, { type, isPrivate: false || null })
        .toArray();
      peopleByTitle.sort(sortAlphabetically);
      req.data.publicPeopleByTitle = publicPeopleByTitle;

      // sorted by recently added
      const publicPeopleByMostRecent = await apos.docs.getManager(type)
        .find(req, { type, isPrivate: false || null })
        .sort({ createdAt: -1 })
        .toArray();
      req.data.publicPeopleByMostRecent = publicPeopleByMostRecent;
    });
  },
};
