...
apos.define('apostrophe-pieces', {
  ...
  afterConstruct: function(self) {
    ...
    // load cms window from in-context
    $('body').on(
      'click',
      '[data-apos-edit-piece]',
      function() {
        editPiece(self);
      }
    );
    $('body').on(
      'click',
      '[data-apos-edit-piece-short-description]',
      function() {
        editPiece(self, 'shortDescription');
      }
    );
    $('body').on(
      'click',
      '[data-apos-edit-piece-slug]',
      function() {
        editPiece(self, 'slug');
      }
    );
...
function editPiece(self, fieldName) {
  var element = $('body')[0];
  var id = element.dataset.aemPieceId;
  var type = element.dataset.aemPieceType;
  if (id && type === self.name) {
    if (fieldName) {
      self.edit(id, { field: fieldName });
    } else {
      self.edit(id);
    }
  }
