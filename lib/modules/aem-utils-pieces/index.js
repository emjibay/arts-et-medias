...
function construct(self, options) {
  ...
  function getFirstNWithTags(pieces, amount, tags) {
    ...
    if (!pieces) {
      log.warning(
        'getFirstNWithTags() expects `pieces`, an array of ApostropheCMS pieces, as first parameter. '
        + 'None was provided, an empty array will be returned.'
      );
      return [];
    }
  ...
  function getFirstTwoWithTags(pieces, tags) {
    ...
    if (!pieces) {
      log.warning(
        'getFirstTwoWithTags() expects `pieces`, an array of ApostropheCMS pieces, as first parameter. '
        + 'None was provided, an empty array will be returned.'
      );
      return [];
    }
  ...
  function hasHeroImage(piece) {
    ...
    if (
      (piece.type === 'article' || piece.type === 'event')
      && piece.isUsingRemoteImage
    ) {
      return true;
      // Default should be `true` in this case.
      // If the remote image does not load,
      // the script in views/partials/scripts/hero-image.njk
      // will remove the <figure> containing the image.
    }
